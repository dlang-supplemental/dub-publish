#!/usr/bin/env nu
# Nushell entry point for dub-publish install (wraps install.ps1 / install.sh).
# Usage:
#   nu install.nu
#   nu install.nu --prefix /custom/path
#   nu install.nu --skip-path
...
def main [
  ...
  if $is_windows {
    ...
    let dir = (
      if ($prefix | is-empty) {
        ($env.LOCALAPPDATA | path join "Programs" "dlang-supplemental" "dub-publish")
      } else { $prefix }
    )
...
def print-nu-path-hint [dir: string] {
  print ""
  print "Nushell — if `dub-publish` is not found in this session:"
