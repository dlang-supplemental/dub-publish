#!/usr/bin/env bash
# Install dub-publish to a per-user prefix and ensure ~/.local/bin is on PATH.
set -euo pipefail
...
echo ""
echo "Credentials (Unix): ~/.dlang-supplemental/dub-publish/password.incoming"
echo "  then: dub-publish login --user YOU --save-credentials"
